...
class TradeHandler(BaseHandler):
    ...
    def post(self):
        ...
        materials = self.get_argument("materials", 0)
        deliver_0_num = self.get_argument("deliver_0_num", 0)
        deliver_0_single = self.get_argument("deliver_0_single", 0)
        deliver_0_time = self.get_argument("deliver_0_time", 0)
        deliver_1_num = self.get_argument("deliver_1_num", 0)
        deliver_1_single = self.get_argument("deliver_1_single", 0)
        deliver_1_time = self.get_argument("deliver_1_time", 0)
        deliver_2_num = self.get_argument("deliver_2_num", 0)
        deliver_2_single = self.get_argument("deliver_2_single", 0)
        deliver_2_time = self.get_argument("deliver_2_time", 0)

        sql = "UPDATE business SET date='%s'" % date
        sql += ", materials=%f" % materials
        sql += ", deliver_0_num=%d" % deliver_0_num
        sql += ", deliver_0_single=%f" % deliver_0_single
        sql += ", deliver_0_time=%d" % deliver_0_time
        sql += ", deliver_1_num=%d" % deliver_1_num
        sql += ", deliver_1_single=%f" % deliver_1_single
        sql += ", deliver_1_time=%d" % deliver_1_time
        sql += ", deliver_2_num=%d" % deliver_2_num
        sql += ", deliver_2_single=%f" % deliver_2_single
        sql += ", deliver_2_time=%d" % deliver_2_time
        sql += " WHERE date='%s'" % date
        trade = self.db.execute(sql)
